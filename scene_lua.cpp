...
//   data->node = new GeometryNode(name, new NonhierBox());
...
int gr_plane_cmd(lua_State* L)
{
  ...
  glm::vec3 origin;
  get_tuple(L, 2, &origin[0], 3);

  glm::vec3 b1;
  get_tuple(L, 3, &b1[0], 3);

  glm::vec3 b2;
  get_tuple(L, 4, &b2[0], 3);

  // double size = luaL_checknumber(L, 5);

  data->node = new GeometryNode(name, new Plane(origin, b1, b2));
...
int gr_area_light_cmd(lua_State* L)
{
  ...
  get_tuple(L, 3, &fall[0], 3);
  
  double b1[3];
  double b2[3];
  get_tuple(L, 4, &b1[0], 3);
  get_tuple(L, 5, &b2[0], 3);

  double size = luaL_checknumber(L, 6);

  const glm::vec3 position(pos[0], pos[1], pos[2]);
  const glm::vec3 colour(col[0], col[1], col[2]);
  const glm::vec3 a(b1[0], b1[1], b1[2]);
  const glm::vec3 b(b2[0], b2[1], b2[2]);

  data->light = new AreaLight(position, a, b, size);
  data->light->colour = colour;
  // data->light->falloff = fall;
...
// Animate a scene
extern "C"
int gr_animate_cmd(lua_State* L)
{
  GRLUA_DEBUG_CALL;
  ...
  double vid_data[2];
  get_tuple(L, 11, vid_data, 2);
  double fps = vid_data[0];
  double duration = vid_data[1];
  int num_frames = fps * duration;

  int effect = 0;
  if (lua_gettop(L) >= 12) {
    effect = luaL_checknumber(L, 12); // effect = 0: normal, 1: final (disco)
  }

  mkdir(filename, 0777);
  Image im(width, height);
  RayTracer rt(root->node, im, eye, view, up, fov, ambient, lights, duration);
  ...
  {
    std::cerr << std::fixed << "Frame " << i << "/" << num_frames << " ("
    << std::setprecision(2) << i*100.0/num_frames << "%) " << std::endl;

    if (i)
    {
      std::chrono::duration<double> elapsed = std::chrono::system_clock::now() - start;
      int secs = elapsed.count() * ( num_frames - i )/i;
      std::cout << "ETA: " << secs/60 << "m " << secs%60 << "s" << std::endl;
    }
    
    rt.render();
    Time::update_time(1/fps);
    root->node->update(1/fps);

    switch(effect)
    {
      case 1: //disco zoom out effect
        rt.disco_update();
        break;
      default:
        break;
  ...
  std::cout << "Finished in " << std::fixed << std::chrono::duration<double>(std::chrono::system_clock::now() - start).count() << " s" << std::endl;
...
// Create a Disco Ball
extern "C"
int gr_disco_ball_cmd(lua_State* L)
{
  GRLUA_DEBUG_CALL;
  ...
  GeometryNode* self = dynamic_cast<GeometryNode*>(data->node);
  ...
  self->set_animator(new Spin(*self));
#ifdef BUMP_MAPPING
  self->m_material->set_bump(new Disco());
#endif
#ifdef AMBIENT_OCCLUSION
  self->m_material->set_occlusion(new DiscoOcclusion());
#endif
  luaL_getmetatable(L, "gr.node");
  lua_setmetatable(L, -2);
...
extern "C"
int gr_water_plane_cmd(lua_State* L)
{
  GRLUA_DEBUG_CALL;
  ...
  luaL_argcheck(L, matdata != 0, 5, "Material expected");

  Material* material = matdata->material;

  self->setMaterial(material);
#ifdef BUMP_MAPPING
  self->m_material->set_bump(new Water());
#endif
  luaL_getmetatable(L, "gr.node");
  lua_setmetatable(L, -2);
...
static const luaL_Reg grlib_functions[] = {
  ...
  {"disco_ball", gr_disco_ball_cmd},
  {"water_plane", gr_water_plane_cmd},
  {"light", gr_light_cmd},
  {"area_light", gr_area_light_cmd},
  {"render", gr_render_cmd},
  {"animate", gr_animate_cmd},
  {0, 0}
};
